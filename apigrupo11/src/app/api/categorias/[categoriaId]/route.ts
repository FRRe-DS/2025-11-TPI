...
export async function GET(_req: NextRequest, { params }: { params: { categoriaId: string } }) {
  const id = Number(params.categoriaId);
...
export async function DELETE(_req: NextRequest, { params }: { params: { categoriaId: string } }) {
  const id = Number(params.categoriaId);
