    ...
    const fetchProductos = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log( 'Productos fetch: ', data);
    ...
    <div style={{ backgroundColor: theme.colors.surface, padding: theme.spacing.lg, borderRadius: theme.borderRadius.lg, boxShadow: theme.shadows.sm, border: `1px solid ${theme.colors.border}` }}>
      <div style={{ display: 'flex', alignItems: 'center' }}>
        <div style={{ padding: '12px', borderRadius: theme.borderRadius.md, background: color }}>
          <Icon style={{ color: theme.colors.textOnPrimary, fontSize: 20 }} />
        </div>
        <div style={{ marginLeft: theme.spacing.md }}>
          <p style={{ margin: 0, fontSize: theme.fontSizes.caption, fontWeight: 600, color: theme.colors.textSecondary }}>{title}</p>
          <p style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800, color: theme.colors.textOnDark }}>{value}</p>
        </div>
      </div>
    </div>
  );
